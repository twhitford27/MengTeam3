...
{
  /* USER CODE BEGIN 1 */
	//double torque_value = calcTORQUE(network_freq, motor_speed, target_speed, fan_k);
	//double torque_ref=10;
	//uint32_t dac_value = (uint32_t) (calcDACvalue(torque_ref, torque_rated));
  /* USER CODE END 1 */
  ...
  while (1)
  {
	 double torque_ref = calcTORQUE(network_freq, motor_speed, target_speed); //calculate Torque reference value in Nm
	 uint32_t dac_value = (uint32_t) (calcDACvalue(torque_ref, torque_rated)); //calculate Torque percentage and adapt to DAC output format
	 HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dac_value); //set DAC value
    /* USER CODE END WHILE */
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
...
uint16_t calcDACvalue(double torque_ref,double torque_rated){
	...
	uint16_t DAC_value_calc = (uint16_t)(4095*torque_percent); //adapt to DAC output value
	return DAC_value_calc;
}
/* USER CODE END 4 */
